...
function sumEvenArguments(){
    return arrayFrom(arguments).reduce((acc, val) => {
        if(val % 2 === 0) {
            return acc+val;
        }
        return acc;
...
function bind(fn, thisArg){
    let argsOuter = arrayFrom(arguments).slice(2);
    return function() {
        let argsInner = arrayFrom(arguments)
        let args = (argsOuter.concat(argsInner));
...
function flip(fn, thisArg){
    ...
    return function() {
        let argsInner = arrayFrom(arguments)
        let args = argsOuter.concat(argsInner).slice(0, fn.length).reverse();
        return fn.apply(thisArg, args)
    }
}
