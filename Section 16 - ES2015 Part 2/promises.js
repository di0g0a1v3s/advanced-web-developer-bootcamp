...
function getMostFollowers(...users){
    return Promise
    .all(users.map(user => $.getJSON(`https://api.github.com/users/${user}`)))
    .then(usersInfo => {
...
function starWarsString(id){
    return $.getJSON(`https://swapi.dev/api/people/${id}`)
    .then(data => {
        return $.getJSON(data.films[0]).then(
            filmData => {
...
function starWarsString(id){
    ...
    .then(data => {
        return $.getJSON(data.films[0]).then(
            filmData => {
                return $.getJSON(filmData.planets[0]).then(
                    planetData => `${data.name} is featured in ${filmData.title}, directed by ${filmData.director} and it takes place on ${planetData.name}`
                )
            }
        )
    })
